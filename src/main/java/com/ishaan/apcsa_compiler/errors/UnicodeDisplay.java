...
/**
 * This implementation is a Java port of <a href="https://github.com/termux/wcwidth/blob/master/wcwidth.c">wcwidth<a> which is distributed under the MIT license
 * This class is an interface that provides a method for determining the width of a unicode codepoint in columns.
 */
public class UnicodeDisplay {
    ...
    private static boolean inTable(int[][] table, int cp) {
    ...
    /**
     * This method calculates the display width of a unicode codepoint in columns.
     *
     * @param codepoint The codepoint whose width is to be calculated.
     * @return The column width of the codepoint.
     */
    public static int getWidth(int codepoint) {
        ...
        if (codepoint < 32 || (0x07F <= codepoint && codepoint < 0x0A0)) return -1;

        // Combining characters with zero width.
        if (inTable(ZERO_WIDTH, codepoint)) return 0;

        return inTable(WIDE_EASTASIAN, codepoint) ? 2 : 1;
    }
}
